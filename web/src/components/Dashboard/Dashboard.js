...
function Dashboard() {
  ...
  const [profile, setProfile] = useState({ avatarUrl: '' });
  ...
  const profileKey = user ? `profile-draft-${user.email}` : null;
  ...
  useEffect(() => {
    ...
    try {
      ...
      setProfile({
        avatarUrl: typeof parsedProfile.avatarUrl === 'string' ? parsedProfile.avatarUrl : '',
      });
    } catch (error) {
      setProfile({ avatarUrl: '' });
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        ...
        <div className="dashboard-header-actions">
          <button onClick={goToProfile} className="dashboard-avatar-button" aria-label="Open profile page">
            {profile.avatarUrl ? (
              <img src={profile.avatarUrl} alt="Profile" className="dashboard-avatar" />
            ) : (
              <div className="dashboard-avatar dashboard-avatar-placeholder">{user.fullName.charAt(0).toUpperCase()}</div>
            )}
          </button>
